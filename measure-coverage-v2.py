...
def read_api_specification(api_specifications_base, api_name):
    f = open(os.path.join(api_specifications_base,
             f'{api_name}-swagger.json'), encoding='UTF-8')
    return json.load(f)
...
if __name__ == '__main__':
    ...
    for api in apis:
        ...
        api_specification_file_path = os.path.join(
            api_specifications_base, f'{api_name}-swagger.json')
        if not os.path.exists(api_specification_file_path):
            print(
                f'API specification file {api_name}-swagger.json not found in {api_specifications_base}. Aborting...')
            exit(1)
